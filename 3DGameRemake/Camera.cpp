...
#include "Time.h"
/*コンストラクタ*/
Camera::Camera() :yaw(0.0f), pitch(0.0f),pendingRecoilYaw(0.0f), pendingRecoilPitch(0.0f), recoilYaw(0.0f), recoilPitch(0.0f), camPos(VGet(0.0f, 0.0f, 0.0f)),recovery(false) {
}
...
void Camera::Update(VECTOR Pos) {
	...
	//コントローラー　
	yaw += stickX * PAD_SENSITIVITY;
	pitch -= stickY * PAD_SENSITIVITY;
